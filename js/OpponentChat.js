/**
 * OpponentChat - Opponent team messaging system
 * Manages chat messages with the opposing team with Firebase Realtime Database integration.
 * Similar to TeamChat but displays messages from the opponent team during gameplay.
 */
...
class OpponentChat {
  ...
  async init() {
    try {
      this.db = getDb();
      if (!this.db) {
        console.warn("[OpponentChat] Database not available during init");
        return;
      }
      this.setupDOM();
  ...
  setupDOM() {
    this.overlay = document.getElementById("opponent-chat-overlay");
  ...
  setupEventListeners() {
    // Use event delegation on body to handle opponent chat buttons
    document.body.addEventListener("click", (e) => {
      const target = e.target.closest("#btn-opponent-chat");
      if (target) {
        e.preventDefault();
        this.openChat();
      }
  ...
  startUnreadCountListener() {
    ...
    try {
      const chatPath = `rooms/${
        gameManager.roomId
      }/opponentChats/${gameManager.getMyTeam()}`;
  ...
  stopUnreadCountListener() {
    if (this.backgroundListener && this.db) {
      try {
        const chatPath = `rooms/${this.currentRoomId}/opponentChats/${this.currentTeam}`;
  ...
  updateUnreadBadge() {
    const badge = document.getElementById("opponent-chat-unread-badge");
  ...
  async openChat() {
    ...
    if (this.teamTitle) {
      this.teamTitle.textContent = `Chat Avversari Squadra ${this.currentTeam}`;
    }
    ...
    const typingPath = `rooms/${gameManager.roomId}/opponentChatTyping/${this.currentTeam}/${gameManager.playerId}`;
  ...
  listenToTyping() {
    ...
    const typingPath = `rooms/${gameManager.roomId}/opponentChatTyping/${this.currentTeam}`;
...
export const opponentChat = new OpponentChat();
